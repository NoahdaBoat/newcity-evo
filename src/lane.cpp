...
void setDefaultTimeEstimate(item ndx) {
  ...
  estimate = std::clamp(estimate, 0.00000000001f, 1.0f/24.f);
...
vec3 getLocation(GraphLocation location) {
  ...
  location.dap = std::clamp(location.dap, 0.f, length);
...
vec3 getLocationV(GraphLocation loc) {
  ...
  float dap = std::clamp(loc.dap/length, 0.f, 1.f);
...
void swapLanesForVehicles(bool fromSave) {
  ...
  for (int i = 1; i < size; i++) {
    ...
    if (fromSave) {
      ...
      blk->timeEstimate = std::clamp(blk->timeEstimate, 0.00000000001f,
