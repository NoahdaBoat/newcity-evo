...
float getTerrainColorU(float z, float nz) {
  ...
  if (z < -100) {
    ...
    u = (z + 100)/100;
    u = std::clamp(u*2+1, 1.f, 3.f);

  } else if (z < beachLine) {
    u = (z - beachLine + 1) / beachLine;
    u = std::clamp(u*2+3, 3.f, 5.f);
    ...
    if (z > snowLine) {
      ...
      float nz = td.n.z;
      nz = 1-nz;
      nz = std::clamp(nz*3, 0, 1);
      //nz = 1-nz*nz;
      nz = 1-nz;
      */

      u = (z - snowLine) / 500;
      u = std::clamp(u, 0.f, 2.f);
      //u *= nz;
      u = std::clamp(u*2+9, 9.f, 11.f);
    } else {
      u = (z - grassLine) / (snowLine - grassLine);
      u = std::clamp(u*2+7, 7.f, 9.f);
    }

  } else if (z > dryGrassLine) {
    z *= nz;
    u = (z - dryGrassLine) / (grassLine - dryGrassLine);
    u = std::clamp(u*2+5, 5.f, 7.f);
...
TileData computeTileData(RenderChunkIndex ndx, item x, item y) {
  ...
  float nz = td.n.z;
  nz = std::clamp(pow(nz*2, 0.2), 0.25, 1.5);
