...
void draw(Shader s) {
  ...
  float illuminationStrength = std::clamp(.8 - light.level, 0., 1.);
  illuminationStrength *= 2;
  if (!isMap && getHeatMap_d() == TrafficHeatMap) illuminationStrength = 2;
  if (s == VehicleShader) illuminationStrength = std::clamp(illuminationStrength, .5f, 1.f);
  ...
  float rain = isShowWeather() ? std::clamp(w.percipitation - 0.1f, 0.f, 1.f) : 0.f;
  if (s == WaterShader) {
  ...
  snow = std::clamp(snow, 0.f, 1.f);

  float pollution = 0.1f * std::clamp(float(light.level)*2-.5f, 0.2f, 1.f);
