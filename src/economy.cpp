...
float getStatisticAt(item econNdx, Statistic stat, float time) {
  ...
  item targetNum = std::clamp(int(round(targetSlice)), 0, numValues);
...
float getStatisticAvg(item econNdx, Statistic stat, float time) {
  ...
  item targetNum = std::clamp(int(round(targetSlice)), 0, numValues-1);
  /*
...
void updateEconomy(float duration) {
  ...
  nationalUnemp = std::clamp(nationalUnemp, 0., 0.3);
  ...
  float bias = mix(c(CTargetUnemploymentBias0), c(CTargetUnemploymentBias1M),
    std::clamp(numPeople(ourCityEconNdx())/1000000.f, 0.f, 1.f));
  ...
  float nextTarget = std::clamp(intrinsicRate + adjustment, 0.01f, 0.4f);
  if (statsReset) {
    targetUnemp = nextTarget;
  } else {
    targetUnemp = mix(targetUnemp, nextTarget, adjRate*20);
  }
  targetUnemp = std::clamp(targetUnemp, 0.f, 0.25f);
  ...
  for (int econ = 1; econ <= econs.size(); econ++) {
    ...
    for (int i = 1; i < numZoneTypes; i++) {
      ...
      if (i == ResidentialZone || i == MixedUseZone) {
        ...
        result *= std::clamp(1 - unemploymentDiff/c(CMaxUnemploymentDiff),
            0.f, 1.f);
      ...
      result = std::clamp(result*2.f, 0.f, 1.f);
