...
void updateAnimation(double duration) {
  ...
  animationTime += std::clamp(dayFrac, 0.f, .25f/24/60);
...
LightInformation getLightInformation() {
  ...
  double lightPower0 = std::clamp(sunHeight * cloudDim, 0., 1.);
  ...
  for (int i = 0; i < 3; i++) {
    ...
    for (int j = 0; j < 4; j++) {
      color += skyColor[j] * std::clamp(float(1-abs(j - pos)), 0.f, 1.f);
    }
  ...
  */
  ...
  float y = 49 + 8*std::clamp(goldAmount, 0., 1.);
  result.color = vec3(
  ...
  result.color *= std::clamp(2 - rz / (sqrt(rx*rx + ry*ry) + 1.f), .5, 2.);
