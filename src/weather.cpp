...
float getIceFade() {
  return showSnow() ? std::clamp(w.temp/-5,0.f,1.f) : 0;
}

float getWaterSnow() {
  return showSnow() ? w.snow * std::clamp((w.temp+5)/-5,0.f,1.f) : 0;
}
...
float getLightning() {
  ...
  return std::clamp(lightning*8-2.9f, 0.f, 1.f);
}
...
void updateWeatherInner(double duration) {
  ...
  //w.temp = std::clamp(w.temp, -40.f, 50.f);
  ...
  w.clouds = std::clamp(clouds, 0.f, 1.f);
  ...
  w.percipitation = std::clamp(w.percipitation, 0.f, w.clouds);
  w.drought = mix(w.drought, 1-w.percipitation, adur);
  w.drought = std::clamp(w.drought, 0.f, 1.f);
  if (w.temp < snowTemp) {
  ...
  w.snow = std::clamp(w.snow, 0.f, 1.f);
  ...
  targetPress = std::clamp(targetPress, -1.f, 1.f);
  w.pressure = mix(w.pressure, targetPress, adur);
  w.pressure += randFloat(-1,1)*adur;
  w.pressure = std::clamp(w.pressure, -1.f, 1.f);
  ...
  nextWindSpeed = std::clamp(nextWindSpeed, 0.1f, 20.f);
...
void updateWeather(double duration) {
  ...
  waterTime += std::clamp(dayFrac, 0.f, .25f/24/60) * (1-getIceFade());
...
float getCurrentRain() {
  return std::clamp(w.percipitation - w.snow, 0.f, 1.f);
}
...
void renderWeather() {
  ...
  if (w.percipitation > 0.1f) {
    ...
    float opacity = std::clamp((w.percipitation-0.1f), 0.f, 1.f);
