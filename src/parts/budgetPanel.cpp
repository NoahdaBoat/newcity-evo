...
Part* budgetPanel() {
  ...
  Budget b[10];
  numBudgets = std::clamp(numBudgets, 0, 3+getNumHistoricalBudgets());
  budgetPage = std::clamp(budgetPage, -getNumHistoricalBudgets(), 1);
  int firstBudget = std::clamp(budgetPage-numBudgets+3,
  ...
  if (firstBudget > -getNumHistoricalBudgets()) {
    Part* pastButt = button(vec2(tableX-scale+bpPadding-1,0.125),
        iconLeft, iconS, moveBudgetPage, -std::clamp(numBudgets, 2, 5));
    r(result, pastButt);
  }

  if (budgetPage < 0) {
    Part* futureButt = button(vec2(tableX-scale+bpPadding,0.125),
        iconRight, iconS, moveBudgetPage, std::clamp(numBudgets, 2, 5));
    r(result, futureButt);
  }
