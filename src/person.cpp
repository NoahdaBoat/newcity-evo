...
void evaluatePerson(item personNdx, float duration) {
  ...
  for (int i = 0; i < locs.size(); i++) {
    ...
    float popFactor = std::clamp(numPeople(econ)/200000.f, 0.f, 10.f);
  ...
  if (c(CEnableMoveOuts) && !isTourist) {
    ...
    */
    ...
    if (family->home != 0 && //getStatistic(NumHomeless) < maxHomeless()*.5f &&
      ...
      socialClass -= std::clamp(timeSinceStore-c(CStoreDays), 0.f, 1.f);
      socialClass -= std::clamp(timeSinceWork-c(CWorkDays), 0.f, 1.f);
      socialClass += 1;
...
void readPerson(FileBuffer* file, int version, item personNdx) {
  ...
  if (version < 45) {
    ...
    wakeTime = std::clamp(wakeTime, time, maxWakeTime);
