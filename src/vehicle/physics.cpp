...
bool updateOneVehicle(item ndx, float duration) {
  ...
  if (trailingNdx > 0) {
    ...
    if (physc.loc.lane/10 == tloc.lane/10) {
      ...
      target = std::clamp(target, 0.f, physLength);
      float advance = target - physc.loc.dap;
      if (advance > 0) {
        physc = moveInLane(ndx, LLPhysical, advance);
      }
  ...
  */
  ...
  if (atDap) {
    ...
    nextStepNdx = std::clamp(float(nextStepNdx), 0.f, laneLength-10);
  }
  ...
  if (nextStep > 0 && nextStepType == LocLaneBlock) {
    ...
    backpressureFactor = std::clamp(backpressureFactor, 0.f, 1.f);
  ...
  advance = std::clamp(advance, 0.f, maxSpeed * duration);
  ...
  if (!atDap && pilot.loc.dap >= laneLength-0.1) {
    if (nextLane == pilot.loc.lane) {
      ...
      //target.dap = std::clamp(target.dap, 0.f, getLaneLength(nextLane));
...
void setTripLimit(item ndx) {
  ...
  tripLimit = std::clamp(tripLimit, float(oneHour), c(CMaxVehicleAge));
